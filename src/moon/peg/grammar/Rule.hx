...
class RuleTools
{
    public static function map(rule:Rule, cb:Rule->Rule):Rule
    {
        return switch (rule)
        {
            // standard PEG rules
            case Seq(a, b):
                cb(Seq(map(a, cb), map(b, cb)));
                
            case Or(a, b):
                cb(Or(map(a, cb), map(b, cb)));
                
            case ZeroOrMore(r):
                cb(ZeroOrMore(map(r, cb)));
                
            case OneOrMore(r):
                cb(OneOrMore(map(r, cb)));
                
            case ZeroOrOne(r):
                cb(ZeroOrOne(map(r, cb)));
                
            case Ahead(r):
                cb(Ahead(map(r, cb)));
                
            case NotAhead(r):
                cb(NotAhead(map(r, cb)));
                
            // non-standard rules
            case Any(a, b):
                cb(Any(map(a, cb), map(b, cb)));
